#ifndef LABA2_1V_RING_BUFFER_H
#define LABA2_1V_RING_BUFFER_H
#include <iostream>
using namespace std;
...
public:
    ...
    friend  bool operator == (const Ring_Buffer<T> &d1, const Ring_Buffer &d2) {
        ...
        int count = d1.Size;
        while (((pres1 != d1.tail)||(count != 0))&&((pres2 != d2.tail)||(count != 0))){
    ...
    friend bool operator >(const Ring_Buffer<T>&d1, const Ring_Buffer<T>&d2) {
        ...
        int cout = d1.Size;
        while (((pres1 != d1.tail)||(count != 0))&&((pres2 != d2.tail) || (count != 0))){
            if (pres1->data < pres2->data)return false;
            if (pres1->data > pres2->data)return true;
            pres1 = pres1->pNext;
            pres2 = pres2->pNext;
            count--;
        }
    ...
    friend std::ostream&operator<<(std::ostream &out, const Ring_Buffer<T>&d) {
        if (d.Size == 0) throw std::out_of_range("RingBuffer is empty");
